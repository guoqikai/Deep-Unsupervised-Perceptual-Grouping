...
def make_overlap_diff_texture(images):
    ...
    t1 = norm_minimax(numpy.real(numpy.fft.ifft2(freq1)))
    t2 = norm_minimax(numpy.real(numpy.fft.ifft2(freq2)))

    img1 = numpy.minimum(images[:half], numpy.expand_dims(numpy.repeat([t1], half, axis=0), axis=3))
    img2 = numpy.minimum(images[half:], numpy.expand_dims(numpy.repeat([t2], half, axis=0), axis=3))
    return numpy.where(img2 == 0, img1, img2)
